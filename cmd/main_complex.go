...
impo		// Authentication flags
		...
		jwtService = flag.String("jwt-service", "", "JWT service name (for service auth)")context"
	"flag"
	"fmt"
	"time"
...
func main() {
	...
	var (
		...
		useLocalKey = flag.Bool("use-local-key", false, "Use local cluster key for authentication")
	...
	if hasJWT {
		if *useLocalKey {
			// For local key authentication, use cluster mode with the provided cluster ID
			opts = append(opts, vizier.WithJWTClusterAuth(*jwtKey, *clusterID))
		} else if *jwtService != "" {
